...
public abstract class AbstractLargestCliqueFinder {
     ...
     */
    public abstract int[] computeLargestClique(Graph graph);
     ...
     */
    protected boolean isClique(Graph graph,
                               List<Integer> clique) {
     ...
     */
    protected int[] getNodeArray(Graph graph) {
        int[] nodeArray = new int[graph.getSize()];
        int index = 0;
     ...
     */
    protected void checkGraphNotEmpty(Graph graph) {
        if (graph.getSize() == 0) {
