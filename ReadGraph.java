...
public class ReadGraph {
    ...
    public static int performExactAlgorithm(int currentNode, int activeColour, boolean[][] a, int[] L2, int[] colours2) {
        ...
        try {
            ...
            if (finished(L2, n)) {
                ...
                boolean[][] conflicts = compareResult(a, L2);
         ...
         * @return boolean[][] list containing 0 values for legal nodes and +1,..,n for every
         * conflict that the node has
         */
        public static boolean[][] compareResult ( boolean adMatrix[][], int[] colourlist){
            // we initialize the list as the adjacency matrix
            boolean[][] conflicts = adMatrix;
            ...
            for (int i = 0; i < adMatrix.length; i++) {
                for (int j = i + 1; j < adMatrix.length; j++) {
                    // if there is adjacency but the color is different then conflict[i][j] = false
	...
	/*public static int nextNode(boolean[][] a, int[] L, int N){
        ...
        public static int[] getNonAdjacentVertices ( int nodeNum, boolean[][] AdMatrix, int[] L2,
        int degreeOftheNode){
